...
//power_up setting :1. pull down all gpio 2.power on regulator 3.pull up standby or reset 4. MCLK 5. IIC
static struct msm_sensor_power_setting imx214_power_setting[] = {
//1.pull down all gpio	
	{
...
//3. MCLK //MCLK pull up before standby and reset for imx214
	{
		.seq_type = SENSOR_CLK,
...
static int32_t imx214_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc;
	if (first_time_power_up_imx214 &&
		msm_sensor_is_probed(s_ctrl->sensordata->sensor_info->position)) {
		pr_info("do not probe imx214,just return\n");
		return -1;
	}
	rc = msm_sensor_power_up(s_ctrl);
	if (0 == rc) {
		pr_info("imx214 probe ok,set first_time_power_up_imx214 to 0.\n");
